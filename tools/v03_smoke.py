...
def main():
    ...
    s,ev,_=c.call('/tier/admin/v1/deployments','POST',{'name':'Synthetic Embeddings','provider_id':pv['id'],'backend_model':'BAAI/bge-m3','capabilities':ep,'enabled':True}); check("embedding deployment",s==201)
    for tier,did in [('Worker',dv['id']),('Embedding-v1',ev['id'])]:
        s,t,h=c.call('/tier/admin/v1/service-levels/'+tier); etag=h.get('ETag'); s,t,_=c.call('/tier/admin/v1/service-levels/'+tier,'PATCH',{'deployment_ids':[did]},headers={'If-Match':etag}); check(f"attach {tier}",s==200)
    probe_results=[c.call('/tier/admin/v1/probes','POST',{'deployment_id':did,'confirm_external_call':True})[1]['status'] for did in (dv['id'],ev['id'])]
    ...
    u=c.call('/tier/v1/usage?'+window,admin=False)[1]; check("principal usage measured",len(u['data'])>=5 and all('measurement_status' in x for x in u['data']))
    snap=u['snapshot_id']; check("stable usage snapshot",c.call('/tier/v1/usage?'+window+'&cursor='+snap+':0',admin=False)[1]['snapshot_id']==snap)
    check("admin ETag conflict",c.call('/tier/admin/v1/providers/'+pv['id'],'PATCH',{'enabled':False},headers={'If-Match':'"wrong"'})[0]==412)
    s,_,_=c.call('/tier/admin/v1/service-levels/Worker','DELETE',headers={'If-Match':'"Worker.v2"'}); check("fixed Tier cannot delete",s==409)
    check("audit and logs",len(c.call('/tier/admin/v1/audit')[1]['data'])>0 and len(c.call('/tier/admin/v1/logs?'+window)[1]['data'])>0)

...
> 2026-09-19 23:30 HKT：放弃 NFS 共享路径，**走 rsync 增量同步 m5air 到 commit `5fb137a`**（本地 191/191 已 PASS）。同步内容：`src/llmtier_v03/*.py` + 新 `tests/{unit,contract,integration,fixtures,system,acceptance,common}/` layout + `docs/std-{lock,source-manifest,tailoring}` + `docs/{00_management,10_requirements,20_system_design,40_module_design,50_implementation_design,60_interfaces,70_verification,80_operations,90_decisions,91_reviews}/` + `tools/contract_semantic_validator_v03.py` + `interfaces/{openapi,compatibility,schemas,vectors}/`。NFS mount (`/Volumes/LLMTier`) 留作只读，mlp 写入被 server 端 squashing 拒，**不修**（server 侧需 `mapall=ben` 等，sudo 不通；m5air 跑老代码已无写需求）。m5air 上 m5air user 装 `jsonschema` 依赖（`pip install --user jsonschema`）。**m5air 191/191 全绿**，PID 11737 跑 commit `5fb137a` 新 src/，OMLX PID 走 provider_local，PID 健康、FD 45、7 tiers ready，smoke `/v1/responses` 200。State 在 `/Users/mlp/LLMTier-dev/state.sqlite3`（NFS 外），NFS mount 路径仅做只读核查，不写。下次依赖 m5air 验证时跑本机 `tests/system/`（待 M2 落地）。
> 2026-09-20 HKT 修订：`2f62b09` 描述说"m5air 未同步"不准确——rsync 实际上在同 session 已经完成，`2f62b09` 是已同步状态下的快照。force-of-habit 在 2f62b09 → 5fb137a → 12c0b54 timeline 里 overwrite 错了。修正见 `12c0b54` 描述：本 commit 真做的是「m5air rsync sync + 全 191/191 + service health OK」。

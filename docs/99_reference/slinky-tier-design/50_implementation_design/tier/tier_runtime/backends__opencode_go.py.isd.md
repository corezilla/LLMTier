# `src/llm_tier/backends/opencode_go.py` ISD

Version: v1.5
Last Updated: 2026-09-08 12:18:44
Status: Draft / Source-Verified Baseline

target_file: `src/llm_tier/backends/opencode_go.py`
...
本ISD以当前`src/llm_tier/backends/opencode_go.py`为行为baseline，目标文件保留其单文件责任；除上游Module Design明确的Approved Delta外，不改变算法、selector、配置路径、重试或fallback。

## 2. 源码结构

- 当前行数：128
- Imports：`__future__:annotations`, `os`, `pathlib:Path`, `typing:Any`, `llm_tier.backends:register_backend`, `llm_tier.backends.deepseek:_api_call`, `llm_tier.backends.base:BaseBackendClient`, `llm_tier.backends.api_backend:ApiBackendMixin`
...
|---|---:|---|---|---|
| `OpenCodeGoClient` | 19 | `class OpenCodeGoClient(BaseBackendClient, ApiBackendMixin)` | none | none |
| `OpenCodeGoClient.__init__` | 26 | `def __init__(self, api_key: str = '', api_key_file: str = '', base_url: str = '', timeout_seconds: int = 600, **kwargs: Any) -> None` | `str.rstrip`, `int`, `str.strip`, `_read_secret_file`, `os.environ.get.strip`, `str`, `os.environ.get` | none |
| `OpenCodeGoClient.backend` | 47 | `def backend(self) -> str` | none | none |
| `OpenCodeGoClient.backend_type` | 51 | `def backend_type(self) -> str` | none | none |
| `OpenCodeGoClient.supported_models` | 60 | `def supported_models(self) -> list[str]` | none | none |
| `OpenCodeGoClient._post_json` | 69 | `def _post_json(self, url: str, payload: dict[str, Any], headers: dict[str, str], timeout: int) -> dict[str, Any]` | `dict`, `enriched_headers.setdefault`, `super._post_json`, `super` | none |
| `OpenCodeGoClient.call` | 87 | `def call(self, model_name: str = '', prompt: str = '', system_prompt: str = '', temperature: float = 0.0, timeout_seconds: int = 600, metadata: dict[str, str] \| None = None, **kwargs: Any) -> dict[str, Any]` | `_api_call` | none |
| `_read_secret_file` | 115 | `def _read_secret_file(path_value: str) -> str` | `str.strip`, `Path.expanduser`, `candidate.is_absolute`, `candidate.read_text.strip`, `str`, `Path`, `Path.cwd`, `candidate.read_text` | none |

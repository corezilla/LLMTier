# `src/llm_tier/backends/cli_io.py` ISD

Version: v1.5
Last Updated: 2026-09-08 12:18:44
Status: Draft / Source-Verified Baseline

target_file: `src/llm_tier/backends/cli_io.py`
...
本ISD以当前`src/llm_tier/backends/cli_io.py`为行为baseline，目标文件保留其单文件责任；除上游Module Design明确的Approved Delta外，不改变算法、selector、配置路径、重试或fallback。

## 2. 源码结构

- 当前行数：207
- Imports：`__future__:annotations`, `os`, `re`, `selectors`, `subprocess`, `time`, `dataclasses:dataclass`, `datetime:datetime,UTC`, `pathlib:Path`, `typing:Any`
...
|---|---:|---|---|---|
| `CliProcessResult` | 21 | `class CliProcessResult` | none | none |
| `build_cli_log_paths` | 36 | `def build_cli_log_paths(*, workspace_root: str \| Path, backend: str, model_name: str, metadata: dict[str, Any] \| None) -> tuple[Path, Path]` | `Path.resolve`, `dict`, `str.strip`, `_safe_path_part`, `datetime.now.strftime`, `any`, `log_dir.mkdir`, `Path`, `stdout_path.parent.mkdir`, `stderr_path.parent.mkdir` | none |
| `run_cli_with_live_logs` | 88 | `def run_cli_with_live_logs(*, command: list[str], cwd: str, env: dict[str, str] \| None, stdin_text: str, timeout_seconds: int, stdout_path: str \| Path, stderr_path: str \| Path) -> CliProcessResult` | `Path`, `stdout_file_path.parent.mkdir`, `stderr_file_path.parent.mkdir`, `stdout_file_path.touch`, `stderr_file_path.touch`, `subprocess.Popen`, `selectors.DefaultSelector`, `selector.register`, `CliProcessResult`, `time.monotonic` | none |
| `_drain_registered_streams` | 175 | `def _drain_registered_streams(selector: selectors.DefaultSelector) -> None` | `list`, `selector.get_map.values`, `selector.unregister`, `key.fileobj.readline`, `chunks.append`, `_append_cli_log`, `selector.get_map` | none |
| `_append_cli_log` | 193 | `def _append_cli_log(file_path: Path, text: str) -> None` | `file_path.open`, `handle.write`, `handle.flush` | none |
| `_safe_path_part` | 205 | `def _safe_path_part(value: str) -> str` | `re.sub`, `str.strip`, `normalized.strip`, `str` | none |

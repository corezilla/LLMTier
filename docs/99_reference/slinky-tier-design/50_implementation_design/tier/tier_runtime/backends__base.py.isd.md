# `src/llm_tier/backends/base.py` ISD

Version: v1.4
Last Updated: 2026-09-08 12:18:44
Status: Draft / Source-Verified Baseline

target_file: `src/llm_tier/backends/base.py`
...
本ISD以当前`src/llm_tier/backends/base.py`为行为baseline，目标文件保留其单文件责任；除上游Module Design明确的Approved Delta外，不改变算法、selector、配置路径、重试或fallback。
...
|---|---:|---|---|---|
| `BaseBackendClient` | 7 | `class BaseBackendClient(ABC)` | none | none |
| `BaseBackendClient.backend` | 10 | `def backend(self) -> str` | none | none |
| `BaseBackendClient.backend_type` | 15 | `def backend_type(self) -> str` | none | none |
| `BaseBackendClient.supported_models` | 19 | `def supported_models(self) -> list[str]` | none | none |
| `BaseBackendClient.call` | 23 | `def call(self, model_name: str, prompt: str, system_prompt: str = '', temperature: float = 0.0, timeout_seconds: int = 600, metadata: dict[str, str] \| None = None, **kwargs: Any) -> dict[str, Any]` | none | none |
...
- 现有相关测试：`test/unit/stage_process/framework/behavior_cases/test_task_identity.py`, `test/unit/stage_process/framework/behavior_cases/test_task_acceptance_artifacts.py`, `test/unit/stage_process/stages/system_testing_stage/behavior_cases/test_execution_step_normalization.py`, `test/unit/stage_process/stages/system_testing_stage/behavior_cases/test_project_entrypoint_facts.py`, `test/unit/stage_process/stages/system_testing_stage/behavior_cases/test_case_data_raw_text_validation.py`, `test/unit/stage_process/stages/system_testing_stage/behavior_cases/test_cross_phase_success_gate_removed.py`, `test/unit/tier/tier_runtime/behavior_cases/test_client_replay.py`, `test/unit/tier/tier_runtime/behavior_cases/test_provider_usage.py`, `test/unit/tier/tier_runtime/behavior_cases/test_mlexp_backend.py`, `test/unit/tier/tier_runtime/behavior_cases/test_server_jobs.py`, `test/unit/memory/indexing/behavior_cases/test_embeddings.py`, `test/unit/memory/retrieval_context/behavior_cases/test_context_provider.py`；其余由Module测试索引覆盖。

# `src/llm_tier/backends/minimax.py` ISD

Version: v1.5
Last Updated: 2026-09-08 12:18:44
Status: Draft / Source-Verified Baseline

target_file: `src/llm_tier/backends/minimax.py`
...
本ISD以当前`src/llm_tier/backends/minimax.py`为行为baseline，目标文件保留其单文件责任；除上游Module Design明确的Approved Delta外，不改变算法、selector、配置路径、重试或fallback。

## 2. 源码结构

- 当前行数：197
- Imports：`__future__:annotations`, `os`, `time`, `urllib.error`, `pathlib:Path`, `typing:Any`, `llm_tier.backends:register_backend`, `llm_tier.backends.api_backend:ApiBackendMixin`, `llm_tier.backends.base:BaseBackendClient`, `llm_tier.exceptions:BackendCallError`
...
|---|---:|---|---|---|
| `_resolve_api_key` | 22 | `def _resolve_api_key(api_key: str = '', api_key_file: str = '') -> str` | `Path.expanduser`, `api_key.strip`, `os.environ.get`, `env_key.strip`, `key_path.is_absolute`, `key_path.read_text.strip`, `Path`, `Path.cwd`, `key_path.read_text` | none |
| `MiniMaxClient` | 48 | `class MiniMaxClient(BaseBackendClient, ApiBackendMixin)` | none | none |
| `MiniMaxClient.__init__` | 55 | `def __init__(self, api_key: str = '', api_key_file: str = '', base_url: str = 'https://api.minimaxi.com/anthropic', timeout_seconds: int = 600, max_tokens: int = 32768, anthropic_version: str = '2023-06-01', **kwargs: Any) -> None` | `_resolve_api_key`, `base_url.rstrip` | none |
| `MiniMaxClient.backend` | 78 | `def backend(self) -> str` | none | none |
| `MiniMaxClient.backend_type` | 88 | `def backend_type(self) -> str` | none | none |
| `MiniMaxClient.supported_models` | 97 | `def supported_models(self) -> list[str]` | none | none |
| `MiniMaxClient.call` | 106 | `def call(self, model_name: str, prompt: str, system_prompt: str = '', temperature: float = 0.0, timeout_seconds: int = 600, metadata: dict[str, str] \| None = None, **kwargs: Any) -> dict[str, Any]` | `time.time`, `int`, `self._api_retry_deadline`, `range`, `BackendCallError`, `self._post_json`, `self._extract_content`, `resp.get`, `kwargs.get`, `self._remaining_api_timeout` | `BackendCallError` |
| `MiniMaxClient._extract_content` | 183 | `def _extract_content(self, resp: dict[str, Any]) -> str` | `resp.get`, `isinstance`, `join`, `texts.append`, `block.get`, `str` | none |

# `src/llm_tier/backends/deepseek.py` ISD

Version: v1.5
Last Updated: 2026-09-08 12:18:44
Status: Draft / Source-Verified Baseline

target_file: `src/llm_tier/backends/deepseek.py`
...
本ISD以当前`src/llm_tier/backends/deepseek.py`为行为baseline，目标文件保留其单文件责任；除上游Module Design明确的Approved Delta外，不改变算法、selector、配置路径、重试或fallback。

## 2. 源码结构

- 当前行数：135
- Imports：`__future__:annotations`, `urllib.error`, `typing:Any`, `llm_tier.backends:register_backend`, `llm_tier.backends.api_backend:ApiBackendMixin`, `llm_tier.backends.base:BaseBackendClient`, `llm_tier.exceptions:BackendCallError`
...
|---|---:|---|---|---|
| `_api_call` | 18 | `def _api_call(self, model_name, prompt, system_prompt, temperature, timeout_seconds, base_url, api_key, max_tokens = 0)` | `time.time`, `messages.append`, `int`, `self._api_retry_deadline`, `range`, `BackendCallError`, `self._post_json`, `choice.get.get`, `resp.get`, `self._remaining_api_timeout` | `BackendCallError` |
| `DeepSeekClient` | 65 | `class DeepSeekClient(BaseBackendClient, ApiBackendMixin)` | none | none |
| `DeepSeekClient.__init__` | 66 | `def __init__(self, api_key: str = '', base_url: str = '', timeout_seconds: int = 300, **kwargs: Any) -> None` | none | none |
| `DeepSeekClient.backend` | 72 | `def backend(self) -> str` | none | none |
| `DeepSeekClient.backend_type` | 76 | `def backend_type(self) -> str` | none | none |
| `DeepSeekClient.supported_models` | 79 | `def supported_models(self) -> list[str]` | none | none |
| `DeepSeekClient.call` | 82 | `def call(self, model_name = '', prompt = '', system_prompt = '', temperature = 0.0, timeout_seconds = 300, metadata = None, **kwargs)` | `_api_call`, `kwargs.get` | none |
| `LocalClient` | 96 | `class LocalClient(BaseBackendClient, ApiBackendMixin)` | none | none |
| `LocalClient.__init__` | 97 | `def __init__(self, api_key: str = '', base_url: str = '', timeout_seconds: int = 120, **kwargs: Any) -> None` | none | none |
| `LocalClient.backend` | 103 | `def backend(self) -> str` | none | none |
| `LocalClient.backend_type` | 107 | `def backend_type(self) -> str` | none | none |
| `LocalClient.supported_models` | 110 | `def supported_models(self) -> list[str]` | none | none |
| `LocalClient.call` | 118 | `def call(self, model_name = '', prompt = '', system_prompt = '', temperature = 0.0, timeout_seconds = 120, metadata = None, **kwargs)` | `_api_call`, `kwargs.get` | none |

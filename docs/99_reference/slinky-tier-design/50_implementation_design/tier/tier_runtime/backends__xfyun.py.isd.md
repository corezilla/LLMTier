# `src/llm_tier/backends/xfyun.py` ISD

Version: v1.5
Last Updated: 2026-09-08 12:18:44
Status: Draft / Source-Verified Baseline

target_file: `src/llm_tier/backends/xfyun.py`
...
本ISD以当前`src/llm_tier/backends/xfyun.py`为行为baseline，目标文件保留其单文件责任；除上游Module Design明确的Approved Delta外，不改变算法、selector、配置路径、重试或fallback。
...
|---|---:|---|---|---|
| `XfyunClient` | 12 | `class XfyunClient(BaseBackendClient, ApiBackendMixin)` | none | none |
| `XfyunClient.__init__` | 13 | `def __init__(self, api_key: str = '', base_url: str = '', timeout_seconds: int = 600, **kwargs: Any) -> None` | `os.environ.get` | none |
| `XfyunClient.backend` | 21 | `def backend(self) -> str` | none | none |
| `XfyunClient.backend_type` | 25 | `def backend_type(self) -> str` | none | none |
| `XfyunClient.supported_models` | 28 | `def supported_models(self) -> list[str]` | none | none |
| `XfyunClient.call` | 37 | `def call(self, model_name: str, prompt: str, system_prompt: str = '', temperature: float = 0.0, timeout_seconds: int = 600, metadata: dict[str, str] \| None = None, **kwargs: Any) -> dict[str, Any]` | `time.time`, `messages.append`, `int`, `self._api_retry_deadline`, `range`, `BackendCallError`, `self._post_json`, `choice.get.get`, `resp.get`, `self._remaining_api_timeout` | `BackendCallError` |

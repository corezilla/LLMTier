# `src/llm_tier/backends/volc.py` ISD

Version: v1.5
Last Updated: 2026-09-08 12:18:44
Status: Draft / Source-Verified Baseline

target_file: `src/llm_tier/backends/volc.py`
...
本ISD以当前`src/llm_tier/backends/volc.py`为行为baseline，目标文件保留其单文件责任；除上游Module Design明确的Approved Delta外，不改变算法、selector、配置路径、重试或fallback。
...
|---|---:|---|---|---|
| `VolcClient` | 18 | `class VolcClient(BaseBackendClient, ApiBackendMixin)` | none | none |
| `VolcClient.__init__` | 25 | `def __init__(self, api_key: str = '', base_url: str = '', timeout_seconds: int = 300, **kwargs: Any) -> None` | `os.environ.get` | none |
| `VolcClient.backend` | 39 | `def backend(self) -> str` | none | none |
| `VolcClient.backend_type` | 49 | `def backend_type(self) -> str` | none | none |
| `VolcClient.supported_models` | 58 | `def supported_models(self) -> list[str]` | none | none |
| `VolcClient.call` | 82 | `def call(self, model_name: str, prompt: str, system_prompt: str = '', temperature: float = 0.0, timeout_seconds: int = 300, metadata: dict[str, str] \| None = None, **kwargs: Any) -> dict[str, Any]` | `time.time`, `messages.append`, `int`, `self._api_retry_deadline`, `range`, `BackendCallError`, `self._post_json`, `choice.get.get`, `resp.get`, `self._remaining_api_timeout` | `BackendCallError` |

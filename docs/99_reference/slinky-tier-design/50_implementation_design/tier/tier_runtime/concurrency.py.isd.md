# `src/llm_tier/concurrency.py` ISD

Version: v1.5
Last Updated: 2026-09-08 12:18:44
Status: Draft / Source-Verified Baseline

target_file: `src/llm_tier/concurrency.py`
...
本ISD以当前`src/llm_tier/concurrency.py`为行为baseline，目标文件保留其单文件责任；除上游Module Design明确的Approved Delta外，不改变算法、selector、配置路径、重试或fallback。

## 2. 源码结构

- 当前行数：154
- Imports：`__future__:annotations`, `collections:defaultdict,deque`, `threading`, `time`, `typing:Any`
...
|---|---:|---|---|---|
| `BackendConcurrencyManager` | 15 | `class BackendConcurrencyManager` | none | none |
| `BackendConcurrencyManager.__init__` | 22 | `def __init__(self, account_config: dict[str, dict[str, Any]] \| None = None) -> None` | `defaultdict`, `threading.Lock` | none |
| `BackendConcurrencyManager.acquire` | 36 | `def acquire(self, backend_key: str, account_key: str) -> tuple[bool, str]` | `str.strip`, `self._account_config.get`, `int`, `self._account_counters.get`, `time.time`, `float`, `max`, `starts.append`, `str`, `cfg.get` | none |
| `BackendConcurrencyManager.release` | 68 | `def release(self, backend_key: str, account_key: str) -> None` | `str.strip`, `self._backend_counters.get`, `self._account_counters.get`, `str` | none |
| `BackendConcurrencyManager.get_load` | 84 | `def get_load(self, backend_key: str) -> int` | `self._backend_counters.get` | none |
| `BackendConcurrencyManager.get_account_load` | 94 | `def get_account_load(self, account_key: str) -> int` | `str.strip`, `self._account_counters.get`, `str` | none |
| `BackendConcurrencyManager.get_account_limit` | 105 | `def get_account_limit(self, account_key: str) -> int` | `str.strip`, `self._account_config.get`, `max`, `str`, `int`, `cfg.get` | none |
| `BackendConcurrencyManager.retry_after_seconds` | 117 | `def retry_after_seconds(self, account_key: str, fallback_seconds: float = 0.5) -> float` | `str.strip`, `max`, `time.time`, `float`, `self._account_config.get`, `str`, `int`, `starts.popleft`, `self._account_next_available_at.get`, `len` | none |
| `BackendConcurrencyManager.set_account_limit` | 148 | `def set_account_limit(self, account_key: str, max_concurrent_requests: int) -> None` | `max`, `str.strip`, `int`, `dict`, `str`, `self._account_config.get` | none |

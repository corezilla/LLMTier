...
> 按 STD `design-data-interface-format` 1.2.0 §3：主章“接口设计”，按**接口形态分类**逐接口完整记录，同一接口只定义一次；标题为真实路由/事件名，标题下先给完整接口声明，再就地说明输入/输出，最后按六项（ID+状态+文件·symbol / 输入 / 成功输出 / 错误与异常 / 交互与生命周期 / 实例与验证）写完。字段级 authority 见 §1。

**类别适用性**：2.1 软件接口 ✓（HTTP 路由）｜2.2 消息与数据流接口 ✓（Responses SSE）｜2.3 硬件与固件接口 ✗（纯软件、无连接器/总线/寄存器/FPGA 端口）｜2.4 人机与维护接口 ✗（无 UI/CLI 操作面；Admin Web UI 归管理控制文档，不在此另立接口）。
...
- **Interface/Member ID、状态、唯一契约、文件·symbol**：`IF-DP-RESPONSES`；规格已定、Implemented；唯一契约=`openapi` candidate.8；文件·symbol `src/http_api/app.py` → `src/inference/responses.py`。
- **输入**：`ResponsesRequest`（§3.4 `D-MSG-RESPONSE`）——`model`（必填，exact 逻辑等级名）、`input`（必填）、`stream`（必须 true）、`store`（必须 false）、`tools`/`tool_choice`/`temperature`/`max_output_tokens`/`reasoning`/`include`/`service_tier`/`metadata`；授权=`data` 角色凭据；校验顺序=鉴权→JSON/schema→形态（stream/store）→模型存在→准入。字段全集见 `openapi` `ResponsesRequest`。
- **成功输出**：`ResponseStreamEvent` 流（§2.2.1/§3.4）；`response.created … response.completed|incomplete|failed`，每请求恰好一个 terminal，`usage` 仅 terminal 给出；副作用=登记用量义务→落账本（§3.6）。
- **错误与异常**：`ERR-REQ-VALIDATION`/`ERR-REQ-FIELD`/`ERR-REQ-JSON`（400，未受理）；`ERR-REQ-UNSUPPORTED`（400，`stream=false`）；`ERR-REQ-TOO-LARGE`（413）；`ERR-AUTH-*`（401/403/503）；`ERR-MODEL-NOTFOUND`（404）；`ERR-RATE-LIMIT`（429 + `Retry-After`）；`ERR-MODEL-UNAVAIL`（503）；`ERR-PROVIDER-UNAVAIL`/`ERR-PROVIDER-FAIL`（502/503）；`ERR-PROVIDER-CONTRACT`（502）；`ERR-INTERNAL`（500）。已建连后断开=结果未知，不创建可恢复 Invocation。
- **交互与生命周期**：同步建连后流式；单请求独立模型调用，无 Agent 会话/工具执行；客户端断开结束本次调用并释放许可；不承诺跨系统 exactly-once；版本固定 candidate.8，无 alias/fallback（§8）。
- **实例与验证**：正常 exact tier → 200 SSE + terminal Usage；拒绝 `stream=false` → 400 `unsupported_request`。向量 `interfaces/vectors/v0.3/openai-surface-fixtures.json`；`VRC-INF-001/002/003/004`。
...
- **Interface/Member ID、状态、唯一契约、文件·symbol**：`IF-DP-EMBEDDINGS`；规格已定、Implemented；唯一契约=`openapi` candidate.8；文件·symbol `src/http_api/app.py` → `src/inference/embeddings.py`。
- **输入**：`EmbeddingRequest`（§3.4 `D-MSG-EMBEDDING`）——`model`（必填，exact embedding tier）、`input`（必填，string 或 string array）、`encoding_format`（默认 `float`）、`dimensions`、`user`；授权=`data` 角色；校验顺序=鉴权→schema→embedding 模型路由（`embedding_space_id` 兼容）→输入/维数/批上限。
- **成功输出**：`EmbeddingResponse` 向量 `data[]` + `usage{prompt_tokens,total_tokens}`；`encoding_format=base64` 时 `embedding` 固定为连续 little-endian IEEE-754 float32 的 RFC 4648 字符串；副作用=登记用量义务→落账本。
- **错误与异常**：`ERR-REQ-VALIDATION`/`ERR-REQ-TOO-LARGE`（400/413）；`ERR-MODEL-NOTFOUND`（404）；`ERR-RATE-LIMIT`（429）；`ERR-PROVIDER-UNAVAIL`/`ERR-PROVIDER-FAIL`（502/503）；`ERR-STORE`（503）。
- **交互与生命周期**：同步；单请求。同一 embedding 逻辑 model 只绑定同一 `embedding_space_id`、模型版本与预处理契约；非兼容变更必须新建逻辑 model ID，由 Slinky 重建索引。
- **实例与验证**：正常 float/base64 返回向量与 Usage；拒绝不兼容维度/数量/索引 → `ERR-REQ-VALIDATION`。向量 `interfaces/vectors/v0.3/openai-surface-fixtures.json`；`VRC-INF-001`。
...
- **Interface/Member ID、状态、唯一契约、文件·symbol**：`IF-DP-MODELS`；规格已定、Implemented；唯一契约=`openapi` candidate.8；文件·symbol `src/inference/models.py`。
- **输入**：路径 `model`（exact-case 逻辑等级名）；授权=`data` 角色；无 body。
- **成功输出**：`D-MODEL`（§3.2）——目录返回 `ModelList`，detail 返回单个 `Model`；只暴露逻辑等级、`availability` 与 `capabilities`（§3.3），不暴露物理账号/provider。
- **错误与异常**：`ERR-AUTH-REQUIRED`/`ERR-AUTH-DENIED`（401/403）；detail 未知 exact 名 → `ERR-MODEL-NOTFOUND`（404）。
- **交互与生命周期**：同步只读；幂等；随 Registry 变更反映；模型列表不分页。
- **实例与验证**：正常返回固定 tier 集合；拒绝未知 exact 名 → 404。向量 `interfaces/vectors/v0.3/admin-model-fixtures.json`；`VRC-INF-001`。
...
- **Interface/Member ID、状态、唯一契约、文件·symbol**：`IF-HEALTH`；规格已定、Implemented；唯一契约=`openapi` candidate.8；文件·symbol `src/http_api/health.py`。
- **输入**：无参数、无凭据。
- **成功输出**：`HealthView`/`ReadinessView`（§3.6）；`/readyz` 由 schema、bootstrap 与固定等级共同决定；副作用=无。
...
- **Interface/Member ID、状态、唯一契约、文件·symbol**：`IF-ADM-PROVIDERS`；规格已定、Implemented；唯一契约=`openapi` candidate.8；文件·symbol `src/management/registry.py`。
...
- **成功输出**：`ProviderView`（§3.2）+ 强 `ETag`；`secret_ref` 只写不回显（view 只给 `has_secret`）；副作用=同事务写审计（§3.6 `D-AUDIT-EVENT`）。
- **错误与异常**：`ERR-AUTH-REQUIRED`/`ERR-AUTH-DENIED`（401/403）；重名/唯一冲突 → `ERR-CONFLICT`（409）；被引用删除 → `ERR-INUSE`（409）；`If-Match` 过期 → `ERR-STALE`（412）；未知 ID → `ERR-NOTFOUND`（404）；`ERR-STORE`（503）。
- **交互与生命周期**：同步；PATCH 为 partial（只更新出现字段，省略保持原值，显式 null 仅在 Schema 允许时清空）；DELETE 幂等；ETag 乐观并发。
- **实例与验证**：正常建 provider 返回 201+ETag；拒绝缺 `If-Match` 的 PATCH → `ERR-STALE`。向量 `interfaces/vectors/v0.3/admin-model-fixtures.json`；`VRC-MGMT-001/002`。
...
- **Interface/Member ID、状态、唯一契约、文件·symbol**：`IF-ADM-PROVIDER-USAGE`；规格已定、Implemented；唯一契约=`openapi` candidate.8；文件·symbol `src/management/account_usage.py`。
- **输入**：路径 `provider_id`；POST body 仅 `{confirm_external_call: bool}`；授权=`admin` 角色。
- **成功输出**：`ProviderAccountUsageSnapshot`（§3.2）——window/percent/used/quota/reset/source/status/checked_at；POST 显式刷新并替换快照；不落 Secret、不回显 credential。
- **错误与异常**：缺二次确认 → `ERR-CONFIRM`（400）；未知 provider → `ERR-NOTFOUND`（404）；`ERR-AUTH-*`（401/403）；上游失败记入快照 `status`（HTTP 200 返回快照事实）。
...
- **Interface/Member ID、状态、唯一契约、文件·symbol**：`IF-ADM-DEPLOYMENTS`；规格已定、Implemented；唯一契约=`openapi` candidate.8；文件·symbol `src/management/registry.py`。
- **输入**：`DeploymentWrite`（`name`/`provider_id`/`backend_model`/`capabilities`/`enabled`）；`DeploymentPatch`（全字段可选）；`If-Match`；授权=`admin`。
- **成功输出**：`DeploymentView`（§3.2）+ ETag；`capabilities` 为 §3.3 `D-CAPABILITY` 12 键；副作用=同事务审计。
...
- **Interface/Member ID、状态、唯一契约、文件·symbol**：`IF-ADM-SERVICE-LEVELS`；规格已定、Implemented；唯一契约=`openapi` candidate.8；文件·symbol `src/management/registry.py`。
- **输入**：`ServiceLevelWrite`（`id` 固定 tier 名/`deployment_ids[]` 有序/`enabled`）；`ServiceLevelPatch`（`deployment_ids`/`enabled`）；`If-Match`；授权=`admin`。
- **成功输出**：`ServiceLevelView`（§3.2）+ ETag；`capabilities` 为成员交集；副作用=同事务审计。
...
- **Interface/Member ID、状态、唯一契约、文件·symbol**：`IF-ADM-PROBES`；规格已定、Implemented；唯一契约=`openapi` candidate.8；文件·symbol `src/management/admin.py`。
...
- **Interface/Member ID、状态、唯一契约、文件·symbol**：`IF-ADM-USAGE`；规格已定、Implemented；唯一契约=`openapi` candidate.8；文件·symbol `src/inference/usage.py`。
- **输入**：GET `from`/`to` 必填，可选 `model`/`request_id`/`cursor`/`limit`；DELETE 过滤参数；GET 可按主体或全部，DELETE 仅 `admin`。
- **成功输出**：`UsagePage`（§3.2，含 `snapshot_id`/`snapshot_at`；measured/estimated/unknown 可区分，unknown 不填零）；DELETE 返回清空结果；副作用=DELETE 改变账本（仅显式 operator 操作）。
- **错误与异常**：`ERR-AUTH-REQUIRED`/`ERR-AUTH-DENIED`（401/403，非 admin DELETE 拒绝）；`ERR-CURSOR`（400）；`ERR-REQ-VALIDATION`（400）；`ERR-STORE`（503，不用空页冒充无记录）；`ERR-REQ-VALIDATION`（400）。
...
- **实例与验证**：正常分页；拒绝非 admin 清空；拒绝过期 cursor。向量 `interfaces/vectors/v0.3/usage-fixtures.json`；`VRC-MGMT-006`。
...
- **Interface/Member ID、状态、唯一契约、文件·symbol**：`IF-ADM-RUNTIME`；规格已定、Implemented；唯一契约=`openapi` candidate.8；文件·symbol `src/inference/routing.py`。
- **输入**：无参数；授权=`admin`。
- **成功输出**：各 deployment 的 in-flight/许可与各 tier FIFO 队列深度快照；副作用=无。
- **错误与异常**：`ERR-AUTH-REQUIRED`/`ERR-AUTH-DENIED`（401/403）。
...
- **Interface/Member ID、状态、唯一契约、文件·symbol**：`IF-ADM-STATS`；规格已定、Implemented；唯一契约=`openapi` candidate.8；文件·symbol `src/management/admin.py`。
...
- **Interface/Member ID、状态、唯一契约、文件·symbol**：`IF-ADM-AUDIT`；规格已定、Implemented；唯一契约=`openapi` candidate.8；文件·symbol `src/management/audit.py`。
- **输入**：`limit`；授权=`admin`。
- **成功输出**：`D-AUDIT-EVENT`（§3.2）列表；不含 prompt/output/Secret。
- **错误与异常**：`ERR-AUTH-REQUIRED`/`ERR-AUTH-DENIED`（401/403）；`ERR-STORE`（503）。
...
- **Interface/Member ID、状态、唯一契约、文件·symbol**：`IF-ADM-LOGS`；规格已定、Implemented；唯一契约=`openapi` candidate.8；文件·symbol `src/log/logs.py`。
- **输入**：`limit`/`level`/`module`/`request_id`/`from`/`to`；授权=`admin`。
- **成功输出**：`D-LOG-EVENT`（§3.2）列表（写前脱敏）；不含 Secret/凭据/完整正文；只读，与管理动作 Audit 分离。
- **错误与异常**：缺 `from`/`to` → `ERR-REQ-VALIDATION`（400）；`ERR-STORE`（503，返回 503 而非空页）。
- **交互与生命周期**：同步只读；7 天保留；稳定快照与逐页授权；store 不可读返回 503。
- **实例与验证**：正常过滤查询；拒绝缺时间窗。向量 `interfaces/vectors/v0.3/admin-model-fixtures.json`；`VRC-LOG-001`。
...
- **Interface/Member ID、状态、唯一契约、文件·symbol**：`IF-MSG-SSE`；规格已定、Implemented；唯一契约=`openapi` `ResponseStreamEvent`；文件·symbol `src/http_api/sse.py`、`src/inference/responses.py`。
- **输入**：一次已受理的 `IF-DP-RESPONSES`（§2.1.1）；输出=SSE 帧；`X-Request-ID` 为 task 头，可接收标准 `traceparent`/correlation context。
- **成功输出**：标准 SSE 事件子集（§3.4 `D-MSG-SSE`）；每 output item 稳定 `id` 与 `output_index`；每请求恰好一个 terminal（`completed|incomplete|failed`）；成功流 terminal response 携带可用 `usage`。
- **错误与异常**：流内失败发 `error` 事件或 `response.failed` terminal；上游失败不伪造完成；客户端断开=结果未知、释放许可（§8）。
- **交互与生命周期**：异步流；从受理到 terminal 的顺序由 `sequence_number` 稳定；`store:false` 不保留 conversation；无 custom recovery/Invocation。
- **实例与验证**：正常完整流以 terminal 结束；边界：上游失败发 `error`/`failed`。向量 `interfaces/vectors/v0.3/openai-surface-fixtures.json`；`VRC-INF-002/005`。
...
#### `ErrorType` / `ErrorCode`（`openapi` `ErrorDetail`）
- **定义、Data/Type ID 与唯一来源**：统一错误信封中的机器分类与公共码值；`D-ERROR-ENVELOPE` 的内层字段；机器源 `openapi` `ErrorDetail`，公共含义由系统设计 §8.8 决定。
- **字段**：`type` ∈ {`invalid_request_error`,`authentication_error`,`permission_error`,`not_found_error`,`conflict_error`,`rate_limit_error`,`provider_error`,`service_unavailable_error`,`internal_error`}；`code` ∈ {`invalid_request`,`authentication_failed`,`permission_denied`,`model_not_found`,`resource_not_found`,`resource_in_use`,`version_conflict`,`rate_limit_exceeded`,`provider_failure`,`service_unavailable`,`usage_store_unavailable`,`internal_error`}。
- **约束 / 不变量**：`additionalProperties:false`；`code` 与 `type` 成对；`ERR-*` 公共 ID 与 wire `code` 的映射见 §4.1；不得自定义未列码。
- **状态 · 所有权 · 寿命**：无状态枚举；随错误响应请求级返回。
- **合法与拒绝实例**：合法 `{type:"provider_error",code:"provider_failure"}`；拒绝未知 `type`/`code` 值。
- **验证**：`openapi` `ErrorDetail`；`VRC-API-002`。

#### `ResponseStatus` / `availability` / `measurement_status` / `source`
- **定义、Data/Type ID 与唯一来源**：跨结构的共享枚举；机器源 `openapi` `ResponsesResponse`、`Model`、`ReadinessView`、`UsageRecord`。
- **字段**：`ResponseStatus` ∈ {`completed`,`failed`,`incomplete`}（流内可 `in_progress`）；`availability` ∈ {`available`,`degraded`,`unavailable`}（`ReadinessView` 与 `Model`）；`measurement_status` ∈ {`measured`,`estimated`,`unknown`}；`source` ∈ {`provider`,`gateway_estimate`,`unavailable`}。
- **约束 / 不变量**：`measurement_status=unknown ⇒ source=unavailable 且 token 全 null`；`measured ⇒ source=provider`；`estimated ⇒ source=gateway_estimate`。
- **状态 · 所有权 · 寿命**：内联于所属结构，随其持久/请求级存在。
- **合法与拒绝实例**：合法 `measurement_status=measured,source=provider`；边界 `unknown` 且 token 全 null（不补零）。
- **验证**：`openapi`；`VRC-INF-004`。

#### `TierId` / `encoding_format`
- **定义、Data/Type ID 与唯一来源**：固定逻辑等级名与 embedding 编码选项；本设计 §3.3/§3.4；机器源 `openapi` `EmbeddingRequest`、`ServiceLevelView`。
- **字段**：`TierId` = 固定集合 {`Senior`,`Junior`,`Worker`,`Associate`,`Engineer`,`Executor`,`Embedding-v1`}；`encoding_format` ∈ {`float`,`base64`}，默认 `float`。
- **约束 / 不变量**：tier `id` 为 exact-case；`base64` 内容固定为连续 little-endian IEEE-754 float32。
- **状态 · 所有权 · 寿命**：tier 名持久于 `service_levels`；`encoding_format` 请求级。
- **合法与拒绝实例**：合法 `Worker`、`float`；拒绝非固定 tier 名或未知 `encoding_format`。
- **验证**：`openapi`；`VRC-INF-001`、`VRC-MGMT-001`。

### 3.2 业务与操作数据结构

#### `ProviderView` / `ProviderWrite` / `ProviderPatch`
- **定义、Data/Type ID 与唯一来源**：provider 资源的写模型与视图；`D-PROVIDER` 的 wire 投影；机器源 `openapi` `ProviderWrite`/`ProviderPatch`/`ProviderView`。
- **字段**：`ProviderWrite{name,kind∈{cloud,local},endpoint(uri),secret_ref:str?,enabled,usage?}`；`ProviderPatch` 同字段全可选且 `minProperties:1`；`ProviderView{id,name,kind,endpoint,has_secret,enabled,usage:ProviderUsageProfileView,request_usage:ProviderRequestUsageView,version:int≥1}`。
- **约束 / 不变量**：`secret_ref` 只写不回显；`name` 唯一；`reply` 中只有 `has_secret`。
- **状态 · 所有权 · 寿命**：SQLite 持久，operator 经 Registry 拥有，带 `version` 乐观并发。
- **合法与拒绝实例**：合法 `{name,kind:cloud,endpoint,secret_ref:"file:/run/secrets/x",enabled:true}`；拒绝明文 `secret_ref="sk-..."` 或重名。
- **验证**：`interfaces/vectors/v0.3/admin-model-fixtures.json`；`VRC-MGMT-001/002`。

#### `DeploymentView` / `DeploymentWrite` / `DeploymentPatch`
- **定义、Data/Type ID 与唯一来源**：deployment 资源模型；`D-DEPLOYMENT`；机器源 `openapi` `DeploymentWrite`/`DeploymentPatch`/`DeploymentView`。
- **字段**：`DeploymentWrite{name,provider_id,backend_model,capabilities:ModelCapabilities,enabled}`；`DeploymentView` 增 `id,health∈{unknown,healthy,degraded,unhealthy},version`。
- **约束 / 不变量**：`provider_id` 必须存在；`capabilities` 为 12 键（§3.3）。
- **状态 · 所有权 · 寿命**：SQLite 持久；operator 经 Registry 拥有。
...
- **定义、Data/Type ID 与唯一来源**：逻辑 tier 资源模型；`D-SERVICE-LEVEL`；机器源 `openapi`。
- **字段**：`ServiceLevelWrite{id,deployment_ids[≥1],enabled}`；`ServiceLevelPatch{deployment_ids?,enabled?}`；`ServiceLevelView` 增 `capabilities,version`。
- **约束 / 不变量**：`id ∈ 7 固定 tier`；`capabilities` = 成员交集；`deployment_ids` 有序。
- **状态 · 所有权 · 寿命**：SQLite 持久，带 `version`。
- **合法与拒绝实例**：合法绑定有序成员；拒绝非固定 tier 名/非法交集。
- **验证**：`openapi`；`VRC-MGMT-001/002`。

#### `UsageRecord` / `UsagePage` / `ProviderAccountUsageSnapshot` / `ProbeResult`
- **定义、Data/Type ID 与唯一来源**：用量与探测的返回结构；`D-USAGE-RECORD`/`D-PROVIDER-SNAPSHOT`；机器源 `openapi` `UsageRecord`/`UsagePage`/`ProviderAccountUsageSnapshot`/`ProbeResult`。
- **字段**：`UsageRecord{request_id,record_version≥1,is_final,model,endpoint∈{/v1/responses,/v1/embeddings},recorded_at,updated_at,measurement_status,source,input_tokens:int?,output_tokens:int?,total_tokens:int?,cached_input_tokens:int?,cache_write_tokens:int?,reasoning_tokens:int?}`；`UsagePage{data[],next_cursor:str?,has_more,snapshot_id,snapshot_at}`；`ProviderAccountUsageSnapshot{provider,source,status∈{ok,unavailable,unsupported,unlimited,not_refreshed},used,quota,remaining,percent,reset_at,window,windows,checked_at,error}`；`ProbeResult{deployment_id,status,checked_at,may_have_incurred_cost}`。
- **约束 / 不变量**：`unknown ⇒ token 全 null`；`has_more=false ⇒ next_cursor=null`；同 request 版本绝不累计。
- **状态 · 所有权 · 寿命**：账本追加式、按 principal 隔离、按保留策略；快照 operator 显式刷新后替换。
- **合法与拒绝实例**：合法 `record_version=2,is_final=true,measurement_status=measured`；边界 `unknown` token 全 null。
- **验证**：`interfaces/vectors/v0.3/usage-fixtures.json`；`VRC-INF-004`、`VRC-MGMT-006`。

#### `AuditEvent` / `AuditPage` / `LogEntry` / `LogPage`
- **定义、Data/Type ID 与唯一来源**：审计与脱敏日志结构；`D-AUDIT-EVENT`/`D-LOG-EVENT`；机器源 `openapi`。
- **字段**：`AuditEvent{id,actor,action,target,result,created_at}`；`LogEntry{id(≤128),created_at,level∈{info,warning,error},module(≤64),event(≤128),message(≤512,写前脱敏),request_id:str?(≤128)}`。
- **约束 / 不变量**：不含 Secret/prompt/output/reasoning/向量/credential/原始 header；`LogEntry.message` 上限 512。
- **状态 · 所有权 · 寿命**：SQLite 持久；审计随管理策略、日志 7 天。
- **合法与拒绝实例**：合法写前脱敏后落库；拒绝含 Secret 原文（须脱敏）。
- **验证**：`interfaces/vectors/v0.3/admin-model-fixtures.json`；`VRC-LOG-001`。

### 3.3 配置与规则数据结构

#### `ModelCapabilities`（`D-CAPABILITY`）
- **定义、Data/Type ID 与唯一来源**：tier/deployment 的 12 键能力固定集合；机器源 `openapi` `ModelCapabilities`。
...
- **约束 / 不变量**：12 键固定且必填；tier 能力 = 成员 deployment 交集。
- **状态 · 所有权 · 寿命**：内嵌 `ProviderView`/`DeploymentView`/`ServiceLevelView` 持久；operator 拥有。
- **合法与拒绝实例**：合法 12 键齐全；拒绝缺键或非交集。
- **验证**：`openapi` `ModelCapabilities`；`VRC-MGMT-*`。

#### `ProviderUsageProfileWrite` / `ProviderUsageProfileView`
- **定义、Data/Type ID 与唯一来源**：provider 账号用量/并发 profile；机器源 `openapi`。
- **字段**：Write{`usage_provider`∈{none,local,minimax,volc},`usage_api_key_ref`?/`usage_access_key_ref`?/`usage_secret_key_ref`?,`max_concurrent_requests`≥1,`min_request_interval_ms`≥0,`requests_per_minute`≥0}；View 以 `has_usage_*` 布尔替代引用值。
- **约束 / 不变量**：引用只写不回显；`usage_provider=none` 时无外部调用。
- **状态 · 所有权 · 寿命**：SQLite 持久；operator 拥有。
- **合法与拒绝实例**：合法 `{usage_provider:minimax,max_concurrent_requests:2}`；拒绝回显 secret。
- **验证**：`openapi`；`VRC-MGMT-*`。

#### `TrustedLanPolicy` / `AuthPolicy`
- **定义、Data/Type ID 与唯一来源**：访问策略规则；本设计 §7；机器源 `openapi` security（`BearerAuth`/`AdminBearerAuth`）。
- **字段**：`DataBearer`/`AdminBearer`（独立）；`trustedLanMode`：客户端在 `192.168.1.0/24` 且开启时可免 Bearer 使用 `trusted-lan-consumer`/`trusted-lan-operator`。
- **约束 / 不变量**：credential 只用于授权，不形成 Client/Source DTO；Admin 与 Data credential 分离；生产使用 TLS。
- **状态 · 所有权 · 寿命**：部署配置，运维拥有。
- **合法与拒绝实例**：合法 `Authorization: Bearer <data-token>`；拒绝缺/越权凭据 → `ERR-AUTH-*`。
- **验证**：`VRC-API-002`。
...
#### `ResponsesRequest` / `ResponsesResponse` / `ResponseStreamEvent`（`D-MSG-RESPONSE`/`D-MSG-SSE`）
- **定义、Data/Type ID 与唯一来源**：OpenAI-compatible Responses 请求/响应/流事件；机器源 `openapi` `ResponsesRequest`/`ResponsesResponse`/`ResponseStreamEvent`。
- **字段（阅读视图）**：`ResponsesRequest{model,store(恒 false),input,stream(恒 true),tools?,tool_choice?,temperature?[0,2],max_output_tokens?≥1,reasoning?,include?,service_tier?,metadata?}`；`ResponsesResponse{id,object:"response",created_at,status,model,output[],usage:TokenUsage?,error:ErrorDetail?}`；`ResponseStreamEvent` 子集见 §2.2.1。
- **约束 / 不变量**：`stream:true`、`store:false` 为受理前提；每请求恰一 terminal；`usage` 仅 terminal 给出；`input` 支持 string 或 item 数组（easy message、assistant 历史、opaque reasoning、function call/output、image tool result）。
...
- **验证**：`interfaces/vectors/v0.3/openai-surface-fixtures.json`；`VRC-INF-001/002`。

#### `EmbeddingRequest` / `EmbeddingResponse` / `EmbeddingItem` / `EmbeddingUsage`（`D-MSG-EMBEDDING`）
- **定义、Data/Type ID 与唯一来源**：OpenAI-compatible Embeddings 请求/响应；机器源 `openapi`。
- **字段（阅读视图）**：`EmbeddingRequest{model,input(string|string[]),encoding_format?,dimensions?,user?}`；`EmbeddingResponse{object:"list",data:[{object:"embedding",index≥0,embedding:number[]|base64}],model,usage:{prompt_tokens,total_tokens}?}`。
- **约束 / 不变量**：请求与响应表示一致；base64 为连续 little-endian IEEE-754 float32；维数须在模型支持集合内；同一逻辑 model 保持同一 `embedding_space_id`。
- **状态 · 所有权 · 寿命**：wire 载荷请求级。
- **合法与拒绝实例**：合法 float/base64 返回向量与 Usage；拒绝不兼容维数/数量/索引。
- **验证**：`interfaces/vectors/v0.3/openai-surface-fixtures.json`；`VRC-INF-001`。

#### `Model` / `ModelList`（`D-MODEL`）
- **定义、Data/Type ID 与唯一来源**：逻辑等级目录条目；机器源 `openapi` `Model`/`ModelList`。
- **字段**：`Model{id,object:"model",created,owned_by:"llmtier",availability,capabilities:ModelCapabilities}`。
- **约束 / 不变量**：`id` exact-case；不暴露物理 provider/account。
- **状态 · 所有权 · 寿命**：只读投影，随 Registry 变更。
- **合法与拒绝实例**：合法返回固定 tier；拒绝未知 exact 名 → `ERR-MODEL-NOTFOUND`。
- **验证**：`interfaces/vectors/v0.3/admin-model-fixtures.json`；`VRC-INF-001`。

#### `ErrorEnvelope`（`D-ERROR-ENVELOPE`，系统拥有含义）
- **定义、Data/Type ID 与唯一来源**：统一错误载荷 `{error:{message,type,code,param}}`；机器源 `openapi` `ErrorEnvelope`/`ErrorDetail`；含义见系统设计 §8.8。
- **字段**：`error.message:str`/`error.type:ErrorType`/`error.code:ErrorCode`/`error.param:str?`（4 字段全必填，`param` 可 null）。
- **约束 / 不变量**：码值语义由 §4.1/系统 §8.8 决定；不含 Secret/凭据/完整正文；429 可带 `Retry-After`。
- **状态 · 所有权 · 寿命**：请求级返回。
- **合法与拒绝实例**：合法 `{error:{type:"model_not_found",code:"model_not_found",param:null}}`；边界：未知端点 → `ERR-NOTFOUND`。
- **验证**：`interfaces/vectors/v0.3/stateless-gateway-boundary-fixtures.json`；`VRC-API-*`。
...
#### `D-USAGE-OBLIGATION` / `D-USAGE-HEAD` / `D-PROVIDER-BINDING`
- **定义、Data/Type ID 与唯一来源**：dispatch 前的用量义务、单调 head 与请求-provider 绑定；本设计 §3.2/系统设计 §8.2；authority `util/migrations/*.sql`。
- **字段**：`D-USAGE-OBLIGATION{principal_id,request_id,model,endpoint,recorded_at,dispatch_authorized_at}`；`D-USAGE-HEAD{principal_id,request_id,head_record_version,updated_at}`；`D-PROVIDER-BINDING{principal_id,request_id,provider_id,deployment_id,bound_at}`。
- **约束 / 不变量**：义务 PK `(principal_id,request_id)` 且必须先于 dispatch；head 单调且 FK 指向存在版本；绑定至多一个。
- **状态 · 所有权 · 寿命**：M003 写；按 principal 隔离；追加式，随账本保留。
- **合法与拒绝实例**：合法：非流式外请求登记 unknown 义务后 dispatch；拒绝：`stream=false` → `ERR-REQ-UNSUPPORTED`（无义务副作用）。
- **验证**：`VRC-INF-004`、`VRC-MGMT-006`。

#### `HealthView` / `ReadinessView`
- **定义、Data/Type ID 与唯一来源**：进程存活与就绪事实；机器源 `openapi`。
- **字段**：`HealthView{status:"ok",version}`；`ReadinessView{status∈{ready,degraded,not_ready},models:[{id,availability∈{available,degraded,unavailable}}]}`。
- **约束 / 不变量**：`/readyz` 由 schema+bootstrap+固定等级共同决定；失败保持 `not_ready` 不接流量。
- **状态 · 所有权 · 寿命**：请求级只读瞬时值。
- **合法与拒绝实例**：正常 `ready`；边界 bootstrap 失败 → 503 `not_ready`。
...
- **约束 / 不变量**：配置带 `version` 乐观并发；账本追加式不可改、同 request 版本绝不累计；审计与 Registry 变更同事务。
- **合法与拒绝实例**：合法：空库由 M007 一次性建表；拒绝：非空库 schema 版本不符 → `ERR-SCHEMA`，拒绝启动。
- **验证**：`util/migrations/*.sql`；`VRC-UTIL-001/002`、`VRC-MGMT-006`。
...
- **约束 / 不变量**：接口逐失败条件只引用上表 `ERR-*`；同码异义、缺码或引用不存在码阻止本契约完成；不自行发明 wire 码。
- **实例与验证**：拒绝 `stream=false` → 400 `unsupported_request`；拒绝未知 tier → 404 `model_not_found`。向量 `interfaces/vectors/v0.3/stateless-gateway-boundary-fixtures.json`；`VRC-INF-001`、`VRC-API-002`。
...
> 本节为跨接口共同政策；逐接口适用条件与结果已回写各接口记录的“交互与生命周期”。分类同 §2：软件接口（HTTP）与消息与数据流接口（SSE）适用；硬件/人机不适用。

- **软件接口（HTTP）**：外部契约不承诺 custom idempotency/exactly-once。内部 admission/queue/concurrency 不暴露资源状态。管理 CRUD 须一致地校验引用并审计；PATCH 为原子局部更新，PATCH/DELETE 使用 ETag/`If-Match`，失败不得留下 partial write。实现并发控制不得扩展 consumer 协议。重复 DELETE 幂等；重复 POST 不保证去重。
...
- **软件接口（HTTP）**：consumer 面 `/v1/*` 与 admin 面随 `openapi`/manifest 显式版本变更；无运行时兼容协商 endpoint；破坏性变化由 LLMTier authority 决定，消费方按新固定基线迁移。

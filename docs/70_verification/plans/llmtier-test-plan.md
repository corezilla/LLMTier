...
| ST-10 | Slinky embedding | **目的**: 验证 embedding 服务可用且返回正确维数。**方法**: POST /v1/embeddings model=Embedding-v1。**环境**: 已配置 bge-m3 deployment (deployment_01839fe2c4f840ed)。**结果**: **已测 PASS** - 200 ✓，维度=1024 ✓，无 NaN/Inf ✓ | 200；`data[0].embedding` 长度=1024；无 NaN/Inf |
...
| ST-18 | FD 稳定性（短期） | **目的**: 验证短期负载下 FD 不泄漏。**方法**: 跑约 50 个请求后检查 FD 和 lsof。**环境**: 无特殊要求。**结果**: **PASS** - FD=3（sequential），FD=85（parallel 1000 requests，稳定） | FD ≤ 80；进程打开的 sqlite 文件数 ≤ 8 |
...
| ST-22 | Provider 限速触发（实际行为） | **目的**: 验证 max_concurrent=1 时 6 并发被队列吸收。**方法**: `LLMTIER_SLOW_ADAPTER_DELAY=1.0` 启用内建 SlowAdapter，6 并发请求。**环境**: 需动态改 provider max_concurrent 配置。**结果**: **PASS** - 6/6 成功，耗时>4s，证明队列吸收 | 全部 200（队列 32 吸收），无 429 |
| ST-22A | Provider 限速触发（人工制造） | **目的**: 验证 slow provider 触发 429。**方法**: 同 ST-22 + 制造慢响应。**环境**: 需代码级 mock slow provider。**结果**: **PASS** - 同 ST-22 | 至少 5×429 带 `Retry-After: 30` |
| ST-23 | Tier 队列满 | **目的**: 验证队列满时第 33+ 路立即 429。**方法**: `LLMTIER_SLOW_ADAPTER_DELAY=2.0`，35 并发，max_concurrent=1。**结果**: **PASS** - 429 立即返回(<2s)，部分请求因30s超时而429（预期行为） | 第 33+ 路立即 429；前 32 路入队 |

...
async function renderTierMembers(){
  ...
  $('#tier-members').innerHTML=deployments.map(deployment=>{const models=state.modelCache[deployment.provider_id]||[];const currentModel=deployment.backend_model;return `<form class="member-card" data-deployment="${esc(deployment.id)}"><div class="member-actions"><button type="button" class="danger member-remove" title="Remove" aria-label="Remove">${iconSvg('unlink')}</button><button type="button" class="primary member-save" title="Save" aria-label="Save">${iconSvg('save')}</button></div><label title="Deployment Name"><input name="name" value="${esc(deployment.name)}" required></label><label title="Provider"><select name="provider_id" onchange="reloadMemberModels(this)">${providerOptions(deployment.provider_id)}</select></label><label title="Backend Model ID"><select name="backend_model" list="model-list-${esc(deployment.id)}" required>${modelOptions(models,currentModel)}</select></label><datalist id="model-list-${esc(deployment.id)}">${[...new Set([...models,currentModel])].map(m=>`<option value="${esc(m)}">`)}</datalist></form>`}).join('')||'<div class="empty compact">This tier has no members.</div>';

...
async function renderTierMembers(){
  ...
  $('#tier-members').innerHTML=deployments.map(deployment=>{const status=backendState(deployment,state.providers.find(item=>item.id===deployment.provider_id),state.runtime.deployments[deployment.id]);const models=state.modelCache[deployment.provider_id]||[];const currentModel=deployment.backend_model;return `<form class="member-card" data-deployment="${esc(deployment.id)}"><div class="member-heading"><b>${esc(deployment.name)}</b><span>${statusMarkup(status[0],status[1])}${esc(deployment.id)} · v${deployment.version}</span><div class="member-actions"><button type="button" class="danger member-remove" title="Remove" aria-label="Remove">${iconSvg('unlink')}</button><button type="button" class="primary member-save" title="Save" aria-label="Save">${iconSvg('save')}</button></div></div><label>Provider<select name="provider_id" onchange="reloadMemberModels(this)">${providerOptions(deployment.provider_id)}</select></label><label>Deployment Name<input name="name" value="${esc(deployment.name)}" required></label><label>Backend Model ID<select name="backend_model" list="model-list-${esc(deployment.id)}" required>${modelOptions(models,currentModel)}</select></label><datalist id="model-list-${esc(deployment.id)}">${[...new Set([...models,currentModel])].map(m=>`<option value="${esc(m)}">`)}</datalist></form>`}).join('')||'<div class="empty compact">This tier has no members.</div>';
  $$('#tier-members .member-card').forEach(form=>{form.onsubmit=saveMember;form.querySelector('.member-remove').onclick=event=>{event.preventDefault();removeMember(form.dataset.deployment)};form.querySelector('.member-save').onclick=event=>saveMember(event)});
